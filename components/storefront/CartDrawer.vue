				...
				<div class="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col z-10">
					...
					<div class="p-4 border-b border-ink-border flex items-center justify-between bg-paper-cream">
						...
						<button type="button"
							...
							aria-label="Close cart drawer" @click="closeDrawer">
							<X :size="18" />
						</button>
					</div>
					...
					<div v-if="items.length === 0"
						...
						<div
							class="w-16 h-16 rounded-full bg-paper-cream flex items-center justify-center text-forest-950">
					...
					<div v-else class="flex-1 overflow-y-auto p-4 space-y-3 divide-y divide-slate-100">
						<div v-for="item in items" :key="`${item.productId}-${item.formatId}`"
							...
							<!-- Cover -->
							<div
								class="w-14 h-18 bg-paper-cream rounded border border-ink-border overflow-hidden flex-shrink-0 flex items-center justify-center">
							...
							<div class="flex-1 min-w-0 space-y-1">
								<div class="flex justify-between items-start gap-2">
									<h5 class="text-xs font-bold text-forest-950 truncate leading-tight">{{ item.title
										}}</h5>
									<button type="button" class="text-red-700 hover:text-red-900 p-0.5 cursor-pointer"
										title="Remove item" @click="removeItem(item.productId, item.formatId)">
										<Trash2 :size="14" />
									</button>
								</div>
								...
								<div class="flex items-center gap-2">
									...
									<span v-if="item.author" class="text-[11px] text-ink truncate">{{ item.author
										}}</span>
								</div>

								<div class="flex justify-between items-center pt-2">
									<QuantityStepper :model-value="item.quantity" :disabled="item.format !== 'hardcopy'"
										size="sm"
					...
					<div v-if="items.length > 0" class="p-4 border-t border-ink-border bg-slate-50 space-y-3">
						...
						<div class="flex items-center gap-2 text-xs text-ink">
							<ShieldCheck :size="16" class="text-emerald-700 flex-shrink-0" />
							<span>Instant eBook downloads &amp; original print copies.</span>
						</div>
