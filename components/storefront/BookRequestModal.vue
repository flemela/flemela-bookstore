...
function handleSubmit(): void {
  ...
  const lines = [
    `*📚 The Sunrise Bookstore — Custom Book Request*`,
...
<template>
	<Teleport to="body">
		<div v-if="open"
			...
			<div class="bg-theme-surface rounded-2xl shadow-2xl border border-theme-border w-full max-w-lg p-6 sm:p-7 space-y-5 animate-in fade-in zoom-in-95 duration-200"
				role="dialog" aria-modal="true">
				<!-- Header -->
				<div class="flex items-center justify-between pb-3 border-b border-theme-border">
					<div class="flex items-center gap-2">
						<div
							class="w-8 h-8 rounded-xl bg-emerald-100 text-emerald-800 flex items-center justify-center">
							<MessageSquare :size="16" />
						</div>
						<div>
							<h3 class="font-display font-bold text-base sm:text-lg text-theme-ink">Request / Special
								Order</h3>
							<p class="text-[11px] text-theme-muted">Our team will source this title and notify you on
								WhatsApp.</p>
						</div>
					</div>
					<button type="button"
				...
				<form class="space-y-4" @submit.prevent="handleSubmit">
					<div class="space-y-1">
						...
						<input v-model="title" type="text" placeholder="e.g. Atomic Habits or Thinking Fast & Slow"
							class="w-full px-3.5 py-2.5 bg-theme-canvas border border-theme-border rounded-xl text-xs sm:text-sm outline-none focus:border-emerald-600 transition-all"
							required />
					</div>
					...
					<div class="grid sm:grid-cols-2 gap-3">
						...
						<div class="space-y-1">
							...
							<input v-model="customerPhone" type="tel" placeholder="07XXXXXXXX or 01XXXXXXXX"
								class="w-full px-3.5 py-2.5 bg-theme-canvas border border-theme-border rounded-xl text-xs sm:text-sm font-mono outline-none focus:border-emerald-600 transition-all"
								required />
						</div>
					</div>
					...
					<div class="space-y-1.5">
						...
						<div class="grid grid-cols-3 gap-2 text-xs font-medium">
							<label class="border rounded-xl p-2.5 text-center cursor-pointer transition-all"
								:class="preferredFormat === 'hardcopy' ? 'border-emerald-600 bg-emerald-50 text-emerald-800 font-bold' : 'border-theme-border hover:bg-slate-50'">
								<input type="radio" value="hardcopy" v-model="preferredFormat" class="sr-only" />
								Physical Hardcopy
							</label>
							<label class="border rounded-xl p-2.5 text-center cursor-pointer transition-all"
								:class="preferredFormat === 'ebook' ? 'border-emerald-600 bg-emerald-50 text-emerald-800 font-bold' : 'border-theme-border hover:bg-slate-50'">
								<input type="radio" value="ebook" v-model="preferredFormat" class="sr-only" />
								eBook (PDF/EPUB)
							</label>
							<label class="border rounded-xl p-2.5 text-center cursor-pointer transition-all"
								:class="preferredFormat === 'either' ? 'border-emerald-600 bg-emerald-50 text-emerald-800 font-bold' : 'border-theme-border hover:bg-slate-50'">
					...
					<div class="pt-2 flex justify-end gap-2.5">
						...
						<button type="submit"
							class="bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold uppercase tracking-wider px-5 py-2.5 rounded-xl transition-all shadow-md flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
