...
function validateAndInitiate(file: File) {
  ...
  if (file.size > maxBytes) {
    currentState.value = 'error';
    errorMessage.value = `File is too large (${formatBytes(file.size)}). Max allowed is ${props.maxFileSizeMb}MB.`;
    return;
  }
...
async function startUploadProcess() {
  ...
  try {
    ...
    emit('success', {
      ...
      fileUrl: negotiateRes.fileUrl || negotiateRes.key,
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        let msg = `Storage upload rejected (Status: ${xhr.status})`;
    ...
    xhr.onerror = () => {
      activeXhr = null;
      reject(new Error('Network drop during upload. Check connection.'));
    };
...
function retryUpload() {
  if (activeFile.value) startUploadProcess();
  else clearFile();
}
