...
const paymentMethodLabel = computed(() => {
  if (orderData.value?.paymentMethod === 'mpesa_cash') return 'Cash on Delivery';
  if (orderData.value?.paymentMethod === 'mpesa_manual') return 'Direct M-Pesa Till';
  return 'Automated M-Pesa';
});
...
const whatsappHelpUrl = computed(() => {
  if (!orderData.value) {
    return buildWhatsAppLink('Hello The Sunrise Bookstore Concierge, I need assistance with my book order.');
  }
  ...
  const text = `Hello The Sunrise Bookstore Concierge, I need assistance with Order #${orderRef}${paymentRef}. Customer: ${orderData.value.customerName}.`;
		...
		<main class="max-w-3xl mx-auto w-full py-8 sm:py-12 px-4 sm:px-6 flex-1">
			<div class="bg-paper-surface rounded-2xl shadow-card border border-paper-border p-6 sm:p-9 space-y-7">
				...
				<div v-if="isPaymentPending && !isCashOrder" class="space-y-6">
					...
					<div v-if="isAutomatedSTK"
						class="bg-paper-cream/60 border border-gold-400/50 rounded-2xl p-6 sm:p-7 space-y-4 shadow-soft">
						<div class="flex items-start gap-4">
							<div class="relative w-11 h-11 flex items-center justify-center flex-shrink-0 mt-0.5">
								...
								<div
									class="w-10 h-10 rounded-full bg-gold-500 text-forest-950 flex items-center justify-center shadow-xs">
									<Zap :size="20" />
								</div>
							</div>
							...
							<div class="space-y-1.5 flex-1">
								<div class="flex items-center justify-between">
									<span
										class="text-[10px] font-mono uppercase font-bold tracking-wider text-gold-600">
										Safaricom STK Prompt Sent
									</span>
									<span
										class="text-xs font-mono font-bold text-forest-950 bg-white px-2 py-0.5 rounded border border-paper-border">
										{{ stkCountdown }}s
									</span>
								</div>
								...
								<p class="text-xs sm:text-sm text-ink-muted leading-relaxed">
									...
									<strong class="text-forest-900 font-mono">{{ orderData ?
										formatCurrency(orderData.total) : 'your order' }}</strong>.
								</p>
							</div>
						</div>
						...
						<!-- Recovery & Status Bar -->
						<div
							class="pt-3 border-t border-paper-border/80 flex flex-wrap items-center justify-between gap-3 text-xs">
							...
							<div class="flex items-center gap-2">
								...
								<button type="button"
									...
									:disabled="isCheckingStatus" @click="handleManualCheckStatus">
					...
					<div v-else-if="isDirectMpesa"
						class="bg-paper-cream/70 border border-gold-400/60 rounded-2xl p-6 sm:p-7 space-y-4 shadow-soft">
						<div class="flex items-start gap-4">
							<div
								class="w-10 h-10 rounded-full bg-gold-500 text-forest-950 flex items-center justify-center shadow-xs flex-shrink-0">
								<Clock :size="20" />
							</div>
							<div class="space-y-1 flex-1">
								<span
									class="text-[10px] font-mono uppercase font-bold tracking-wider text-gold-600 block">
								...
								<p class="text-xs sm:text-sm text-ink-muted leading-relaxed">
									Thank you, <strong>{{ orderData?.customerName }}</strong>. Our store administrator
									is validating your reference code against our till records.
								</p>
							</div>
						</div>

						<div
							class="bg-white rounded-xl p-4 border border-paper-border text-xs text-forest-950 space-y-2 shadow-2xs">
							<div class="flex justify-between items-center text-ink-muted">
								<span>Submitted Code:</span>
								<span
									class="font-mono font-bold text-forest-950 text-sm tracking-wider uppercase bg-slate-100 px-2.5 py-1 rounded border border-slate-200">
						...
						<div class="flex flex-wrap items-center justify-between gap-3 pt-1">
							...
							<div class="flex items-center gap-2">
								...
								<button type="button"
									class="px-3.5 py-1.5 bg-forest-950 hover:bg-forest-900 text-paper text-xs font-bold rounded-lg flex items-center gap-1.5 shadow-subtle cursor-pointer transition-all disabled:opacity-50"
									:disabled="isCheckingStatus" @click="handleManualCheckStatus">
									<RefreshCw :size="12" :class="{ 'animate-spin': isCheckingStatus }" />
									<span>Check Status</span>
								</button>
				...
				<!-- STATE B: PAYMENT REJECTED / FAILED -->
				<div v-else-if="isPaymentFailed"
					class="bg-red-50 border border-red-200 rounded-2xl p-6 sm:p-7 space-y-4 shadow-soft">
					<div class="flex items-start gap-4">
						...
						<div class="space-y-1 flex-1">
							...
							<p class="text-xs sm:text-sm text-red-800 leading-relaxed">
								The M-Pesa prompt was not approved. You can switch to another payment method or
								re-attempt with your phone number below without re-entering your order details.
							</p>
						</div>
					</div>
				...
				<!-- RECOVERY DRAWER: In-Place Payment Switcher -->
				<div v-if="showPaymentRecovery"
					class="p-5 bg-paper-cream/70 border border-gold-400 rounded-2xl space-y-4 shadow-soft animate-in fade-in">
					<div class="flex items-center justify-between border-b border-paper-border pb-2">
						...
						<button type="button" class="text-xs text-ink-muted underline cursor-pointer"
					...
					<div class="space-y-2.5 text-xs">
						...
						<div v-if="recoveryMethod === 'mpesa'" class="pl-6 space-y-1">
							<input v-model="recoveryPhone" type="tel" placeholder="07XXXXXXXX or 01XXXXXXXX"
						...
						<div v-if="recoveryMethod === 'mpesa_manual'" class="pl-6 space-y-1">
							<input v-model="recoveryMpesaCode" type="text"
					...
					<div class="pt-2 flex justify-end gap-2">
						<button type="button"
							...
							:disabled="isSubmittingRecovery" @click="handleRetryOrSwitchPayment">
							{{ isSubmittingRecovery ? 'Submitting...' : 'Submit & Proceed' }}
						</button>
					</div>
				</div>

				<!-- STATE C: PAYMENT ACCEPTED (CONFIRMATION) -->
				<div v-if="isPaymentPaid || isCashOrder || orderData?.status === 'confirmed' || orderData?.status === 'delivered'"
					class="space-y-6">

					<!-- 1. Verified Payment Badge -->
					<div v-if="isPaymentPaid && !isCashOrder"
						class="text-center space-y-2 pb-5 border-b border-paper-border">
						<div
						...
						<p class="text-xs sm:text-sm text-ink-muted max-w-md mx-auto">
							Thank you, <strong>{{ orderData?.customerName }}</strong>. Your reading purchase has been
							approved and confirmed.
						</p>
					</div>

					<!-- 2. Automatic Email Confirmation Dispatch Notice -->
					<div v-if="hasDigitalDownloads && isPaymentPaid"
						...
						<div>
							<strong class="font-semibold text-emerald-900 block">Permanent Backup Dispatched via
								Email</strong>
							<span class="text-emerald-950/80">
								A permanent receipt with direct eBook recovery links has been sent to <strong>{{
									customerEmailDisplay }}</strong>.
							</span>
						</div>
					</div>

					<!-- 3. Digital Downloads Vault (Instant direct downloads) -->
					<div v-if="hasDigitalDownloads" class="space-y-3.5">
						<div class="flex items-center justify-between">
							<h3
								class="font-display font-bold text-sm text-forest-950 uppercase tracking-wider flex items-center gap-2">
								<Download :size="16" class="text-emerald-700" /> Your Digital Editions (Direct Download)
							</h3>
							<span
						...
						<div class="grid sm:grid-cols-2 gap-3.5">
							<div v-for="dl in orderData!.downloads" :key="dl.token"
								...
								<div class="space-y-1">
									<span
										class="text-[9px] font-mono font-extrabold uppercase px-2 py-0.5 rounded-md bg-forest-950 text-gold-300">
										{{ dl.format.toUpperCase() }} EBOOK
									</span>
									<h4 class="text-xs sm:text-sm font-bold text-forest-950 mt-1 line-clamp-1">{{
										dl.bookTitle }}</h4>
									<p class="text-[11px] text-ink-muted">Downloads remaining: {{ dl.maxDownloads -
										dl.downloadCount }}</p>
								</div>

								<a :href="`/api/books/download/${dl.token}?redirect=true`" target="_blank"
					...
					<div v-if="orderData?.deliveryConfirmationCode && orderData?.deliveryType === 'delivery'"
						class="p-5 sm:p-6 bg-paper-cream/50 border-2 border-dashed border-emerald-600/50 rounded-2xl text-center space-y-2 shadow-soft">
						<span
							class="text-xs font-bold uppercase text-forest-950 flex items-center justify-center gap-1.5 tracking-wider font-mono">
							<KeyRound :size="15" class="text-emerald-700" /> Courier Delivery Handover Code
						</span>
						<div class="flex items-center justify-center gap-3">
							<span
								class="font-mono text-2xl sm:text-3xl font-extrabold text-forest-950 tracking-widest bg-white px-6 py-1.5 rounded-xl border border-paper-border shadow-xs">
							...
							<button type="button"
								...
								title="Copy verification code" @click="copyConfirmationCode">
								<component :is="isCodeCopied ? Check : Copy" :size="16"
									:class="isCodeCopied ? 'text-emerald-700' : 'text-forest-950'" />
							</button>
						</div>
					...
					<div class="border-t border-paper-border pt-4 space-y-2.5 text-xs">
						<div class="flex justify-between items-center text-ink-muted">
							...
							<span class="font-mono font-bold text-forest-950">#{{ orderData?.orderId.slice(0,
								8).toUpperCase() }}</span>
						</div>
						<div class="flex justify-between items-center text-ink-muted">
						...
						<div v-if="orderData?.paymentReference"
							class="flex justify-between items-center text-ink-muted">
							<span>M-Pesa Reference</span>
							<span
								class="font-mono font-bold text-forest-950 uppercase bg-slate-100 px-2 py-0.5 rounded border">
								{{ orderData.paymentReference }}
							</span>
						</div>
						<div class="flex justify-between items-center text-ink-muted">
							<span>Fulfillment Location</span>
							<span class="font-medium text-forest-950 truncate max-w-xs">{{ orderData?.deliveryLocation
								}}</span>
						</div>
						<div
							class="flex justify-between items-baseline pt-2.5 border-t border-paper-border text-sm font-bold text-forest-950">
							<span>Total Amount</span>
							<span
								class="font-display font-extrabold text-forest-900 font-mono tabular-figure text-base sm:text-lg">
								{{ orderData ? formatCurrency(orderData.total) : '' }}
							</span>
						</div>
					</div>
					...
					<div class="pt-2 flex flex-wrap gap-3.5">
						...
						<NuxtLink to="/"
							...
							<ShoppingBag :size="15" class="text-gold-300" /> Return to Bookstore
							<ArrowRight :size="14" />
						</NuxtLink>
					</div>
				</div>
