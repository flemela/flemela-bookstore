...
async function loadHeroNotes(): Promise<void> {
  try {
    const data = await $fetch<StoreHeroNotes>('/api/admin/hero-notes');
    if (data) {
...
async function handleSaveHeroNotes(updated: StoreHeroNotes): Promise<void> {
  isSavingNotes.value = true;
  try {
    const res = await $fetch<StoreHeroNotes>('/api/admin/hero-notes', {
      method: 'PUT',
      body: updated,
    });
    heroNotes.value = res;
    pushToast({ message: 'Hero editorial notes updated!', variant: 'success' });
  } catch (err: any) {
    pushToast({
      message: err.data?.statusMessage || err.statusMessage || 'Failed to save hero notes',
      variant: 'error',
    });
  } finally {
...
async function handleSaveBanner(): Promise<void> {
	...
	<AdminLayout>
		<div class="space-y-6 max-w-6xl mx-auto">
			...
			<div class="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-paper-border">
				<div>
					<span
						class="text-[9px] sm:text-[10px] font-mono uppercase tracking-widest text-gold-600 font-bold block">
			...
			<div v-if="activeTab === 'ticker'" class="space-y-6 animate-in fade-in duration-200">
				...
				<div class="bg-paper-surface p-5 rounded-2xl border border-paper-border shadow-soft space-y-3">
					...
					<div class="rounded-xl overflow-hidden py-3 px-4 border border-[#916515] text-[#052219] font-sans font-extrabold text-xs sm:text-sm text-center shadow-xs"
						:style="{
              background: 'linear-gradient(135deg, #BF953F 0%, #FCF6BA 25%, #B38728 50%, #FBF5B7 75%, #AA771C 100%)',
				...
				<div class="bg-paper-surface rounded-2xl border border-paper-border shadow-soft p-6 space-y-5">
					...
					<div v-if="!tickerItems.length" class="text-center py-8 text-xs text-ink-muted space-y-2">
						...
						<button type="button" class="text-forest-900 font-bold underline cursor-pointer"
							@click="addTickerItem">
					...
					<div v-else class="space-y-3">
						<div v-for="(item, idx) in tickerItems" :key="item.id"
							...
							<span
								class="w-6 h-6 rounded-full bg-paper-cream text-forest-950 font-mono font-bold text-xs flex items-center justify-center flex-shrink-0 border border-paper-border">
								{{ idx + 1 }}
							</span>
							...
							<div class="w-full sm:w-56 space-y-1">
								<input v-model="item.link" type="text" placeholder="Link (e.g. #flash-sale)"
							...
							<div class="flex items-center gap-2 self-end sm:self-center">
								...
								<button type="button"
									...
									title="Remove message" @click="removeTickerItem(idx)">
									<Trash2 :size="15" />
								</button>
							</div>
						</div>
					...
					<div class="pt-3 border-t border-paper-border flex justify-end">
						<button type="button"
							...
							:disabled="isSavingTicker" @click="handleSaveTicker">
			...
			<div v-else-if="activeTab === 'banners'" class="space-y-6 animate-in fade-in duration-200">
				<div class="flex justify-end gap-2.5">
					<button type="button"
						...
						<RefreshCw :size="13" :class="{ 'animate-spin': bannersStatus === 'pending' }" />
				...
				<div class="bg-paper-surface rounded-2xl border border-paper-border shadow-soft overflow-hidden">
					<div v-if="bannersStatus === 'pending'" class="p-12 text-center text-xs text-ink-muted">
						Loading active banners...
					</div>

					<div v-else-if="!banners?.length" class="p-12 text-center space-y-3">
						<div
							class="w-12 h-12 bg-paper-cream rounded-full flex items-center justify-center text-forest-900 mx-auto">
							<Images :size="24" />
						</div>
						...
						<div v-for="(banner, index) in banners" :key="banner.id"
							...
							<div class="flex items-center gap-3 w-full sm:w-auto">
								<div class="flex flex-col gap-1 text-ink-muted">
									<button type="button"
										...
										:disabled="index === 0 || isReordering" title="Move banner up"
									...
									<button type="button"
										...
										title="Move banner down" @click="moveBanner(index, 'down')">
										<ArrowDown :size="13" />
									</button>
								</div>

								<div
									class="w-32 sm:w-40 aspect-[4/1] rounded-lg border border-paper-border overflow-hidden bg-forest-950 flex-shrink-0 shadow-xs relative">
									<img :src="banner.image_url" :alt="banner.title || 'Banner'"
										class="w-full h-full object-cover" />
									<span v-if="banner.badge"
								...
								<div class="space-y-1 min-w-0 flex-1">
									<div class="flex items-center gap-2">
										...
										<span class="text-[9px] font-mono font-bold uppercase px-1.5 py-0.2 rounded"
											:class="banner.is_active ? 'bg-emerald-100 text-emerald-900' : 'bg-slate-100 text-slate-700'">
											{{ banner.is_active ? 'Active' : 'Paused' }}
										</span>
									</div>
									<p v-if="banner.subtitle" class="text-[11px] text-ink-muted line-clamp-1">{{
										banner.subtitle }}</p>
									<div class="flex items-center gap-3 text-[10px] text-ink-subtle font-mono">
										<span v-if="banner.cta_link">Target: <strong>{{ banner.cta_link
												}}</strong></span>
							...
							<div class="flex items-center gap-2 self-end sm:self-center">
								...
								<button type="button"
									...
									title="Delete banner" @click="handleDelete(banner.id)">
									<Trash2 :size="14" />
								</button>
							</div>
						</div>
		...
		<Teleport to="body">
			<div v-if="showModal"
				...
				<div
					class="bg-white rounded-2xl shadow-2xl border border-paper-border max-w-lg w-full p-6 sm:p-7 space-y-5 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
					...
					<form class="space-y-4" @submit.prevent="handleSaveBanner">
						<div class="space-y-1.5">
							...
							<div class="flex gap-2 items-center">
								<input v-model="form.image_url" type="text" placeholder="https://..."
									...
									required />
								<label
									class="bg-paper-cream border border-paper-border hover:bg-slate-200 px-3 py-2 rounded-xl text-xs font-bold text-forest-950 flex items-center gap-1 cursor-pointer flex-shrink-0">
									<Upload :size="13" />
									<span>{{ isUploadingDesktop ? 'Uploading...' : 'Upload' }}</span>
									<input type="file" accept="image/*" class="hidden" :disabled="isUploadingDesktop"
										@change="handleImageUpload($event, 'image_url')" />
								</label>
							</div>

							<div v-if="form.image_url"
								class="relative rounded-lg border border-paper-border overflow-hidden aspect-[4/1] bg-forest-950 mt-1.5">
								...
								<button type="button"
									...
									title="Remove image" @click="form.image_url = ''">
									<X :size="12" />
								</button>
							</div>
						</div>
						...
						<div class="space-y-1.5 pt-1">
							<div class="flex justify-between items-baseline">
								...
								<span
									class="text-[10px] text-ink-muted font-mono font-semibold uppercase tracking-wide">
									Target: 1.65:1 (390Ã—240px)
								</span>
							</div>

							<div class="flex gap-2 items-center">
								<input v-model="form.mobile_image_url" type="text" placeholder="https://..."
									class="flex-1 px-3 py-2 border border-paper-border rounded-xl text-xs outline-none focus:border-forest-900" />
								<label
									class="bg-paper-cream border border-paper-border hover:bg-slate-200 px-3 py-2 rounded-xl text-xs font-bold text-forest-950 flex items-center gap-1 cursor-pointer flex-shrink-0">
									<Upload :size="13" />
									<span>{{ isUploadingMobile ? 'Uploading...' : 'Upload' }}</span>
									<input type="file" accept="image/*" class="hidden" :disabled="isUploadingMobile"
						...
						<div class="space-y-3 pt-2 border-t border-paper-border">
							...
							<div class="space-y-1">
								...
								<textarea v-model="form.subtitle" rows="2" placeholder="Leave empty if not needed"
						...
						<div class="pt-3 flex justify-end gap-2.5 border-t border-paper-border">
							...
							<button type="submit"
								...
								:disabled="isSavingBanner || isUploadingDesktop || isUploadingMobile">
								{{ isSavingBanner ? 'Saving...' : (editingBannerId ? 'Update Banner' : 'Create Banner')
								}}
							</button>
						</div>
					</form>
