					...
					<form @submit.prevent="handleUpdate" class="space-y-6">
						...
						<div v-if="successToast"
							...
							<button type="button" @click="successToast = ''"
								class="text-xs text-emerald-700 font-bold cursor-pointer">Dismiss</button>
						</div>

						<!-- Error Alert -->
						<div v-if="formError"
							class="p-4 rounded-xl bg-red-50 border border-red-200 flex items-start space-x-3">
						...
						<div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
							<div class="flex items-center justify-between border-b border-gray-100 pb-3">
								...
								<button type="button" :disabled="isFindingCover || !form.name.trim()"
									...
									<svg v-if="isFindingCover" class="animate-spin w-3.5 h-3.5 text-amber-900"
							...
							<div class="flex flex-col sm:flex-row items-start gap-6">
								<div
									class="w-32 h-44 bg-gray-100 rounded-xl border border-gray-200 overflow-hidden flex-shrink-0 flex items-center justify-center relative shadow-sm">
									<img v-if="form.cover_image_url" :src="form.cover_image_url"
										alt="Book cover preview" class="w-full h-full object-cover"
							...
							<div class="p-5 rounded-xl border border-gray-200 bg-gray-50/30 space-y-4">
								<div class="flex items-center justify-between">
									<h3 class="text-sm font-bold text-gray-900 flex items-center gap-1.5">
										...
										<span class="text-[10px] font-mono text-gray-500 uppercase">(Doorstep Delivery /
											Pickup)</span>
									</h3>
									<span v-if="hardcopyFormatId"
										class="text-xs text-gray-600 font-mono bg-gray-100 px-2 py-0.5 rounded">
										Format ID: {{ hardcopyFormatId.slice(0, 8) }}
									</span>
									<span v-else
										class="text-[10px] font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded">
								...
								<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
									<div>
										<label class="block text-xs font-semibold text-gray-700 mb-1">Selling Price
											(KSh) *</label>
										<input v-model.number="form.hardcopyPrice" type="number" min="0"
											placeholder="e.g. 999"
									...
									<div>
										<label class="block text-xs font-semibold text-gray-700 mb-1">Current Stock
											Count</label>
										<input v-model.number="form.hardcopyStock" type="number" min="0"
											class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm font-mono" />
									</div>
								</div>
							</div>
							...
							<div class="p-5 rounded-xl border border-gray-200 bg-gray-50/30 space-y-4">
								<div class="flex items-center justify-between">
									...
									<span v-if="pdfFormatId"
										class="text-xs text-emerald-800 font-bold bg-emerald-50 px-2 py-0.5 rounded">
										Format Active (ID: {{ pdfFormatId.slice(0, 8) }})
									</span>
									<span v-else-if="form.pdfKey"
										class="text-[10px] font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded">
										Will be generated upon save
									</span>
								</div>
								...
								<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
									<div>
										<label class="block text-xs font-semibold text-gray-700 mb-1">eBook Price
											(KSh)</label>
										<input v-model.number="form.pdfPrice" type="number" min="0"
											placeholder="e.g. 149"
											class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm font-mono font-bold" />
									</div>
								...
								<div v-if="isPdfDirty"
									class="p-3 bg-emerald-50 border border-emerald-200 rounded-lg flex items-center space-x-2">
									<svg class="w-4 h-4 text-emerald-700 flex-shrink-0" fill="none" viewBox="0 0 24 24"
										stroke="currentColor">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
											d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
									</svg>
									<p class="text-[11px] text-emerald-800 font-medium">
										New PDF ready for upload. Click <strong>Save Changes</strong> to update the
